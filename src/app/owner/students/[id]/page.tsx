...
export default function StudentDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <Navigation>
        <div className="flex flex-col items-center justify-center p-24 animate-pulse">
          <div className="w-16 h-16 rounded-full bg-[var(--md-sys-color-primary-container)] mb-4" />
          <p>Memuat profil detail atlet...</p>
        </div>
      </Navigation>
    );
  }
  ...
  const currentBeltIdx = beltOrder.indexOf(student.current_belt);
    ...
    <Navigation>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          <a href="/owner/students" className="m3-btn-outlined py-1.5 px-3 text-xs font-semibold">
            ← Kembali ke Daftar
          </a>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="m3-card-elevated flex flex-col items-center text-center">
            ...
            <div className="w-full grid grid-cols-2 gap-2 text-left mt-6 pt-6 border-t" style={{ borderColor: 'var(--md-sys-color-outline-variant)' }}>
              <div>
                <span className="block text-[10px] opacity-60">Status Kehadiran</span>
                <span className="font-semibold text-sm">{attPct}% ({hadirAtt}/{totalAtt})</span>
              </div>
              <div>
                <span className="block text-[10px] opacity-60">Tanggal Join</span>
          ...
          <div className="m3-card-elevated md:col-span-2">
            ...
            <div className="space-y-1.5 mb-6">
              ...
              <div className="w-full h-3 rounded-full bg-[var(--md-sys-color-surface-container-highest)] overflow-hidden">
                <div className="h-full rounded-full transition-all duration-700 bg-[var(--md-sys-color-primary)]" style={{ width: `${progressPct}%` }} />
              </div>
            </div>
                ...
                return (
                  <div key={belt} className="flex items-center flex-shrink-0">
                    ...
                    {idx < beltOrder.length - 1 && (
                      <div className={`w-6 h-0.5 ${isPassed ? 'bg-emerald-500' : 'bg-black/10'}`} />
                    )}
                  </div>
                );
              })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {attendance.slice(0, 5).map(rec => {
                  ...
                  return (
                    <div key={rec.id} className="py-2.5 flex items-center justify-between">
                      ...
                      <span className="text-xs font-bold capitalize" style={{ color: 'var(--md-sys-color-primary)' }}>{rec.status}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="m3-card-elevated flex flex-col justify-between">
            <div>
              ...
              <p className="text-[10px] opacity-70 mb-3">Informasi riwayat medis, cedera, atau evaluasi teknik khusus atlet</p>
              <textarea
                value={coachNotes}
                onChange={e => setCoachNotes(e.target.value)}
                placeholder="Tulis evaluasi atau riwayat medis..."
                rows={5}
                className="w-full text-xs p-3 rounded-xl m3-textfield-outlined border bg-transparent"
                ...
                {examsPlayed.map(ep => (
                  <div key={ep.id} className="p-3 rounded-xl flex items-center justify-between" style={{ background: 'var(--md-sys-color-surface-container-high)' }}>
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold capitalize"
                      style={ep.result === 'lulus' ? { background: 'var(--md-sys-color-tertiary-container)', color: 'var(--md-sys-color-on-tertiary-container)' } : { background: 'var(--md-sys-color-error-container)', color: 'var(--md-sys-color-on-error-container)' }}>
                      {ep.result}
                    </span>
                  </div>
                ))}
                ...
                {tournamentRuns.map(tr => (
                  <div key={tr.id} className="p-3 rounded-xl flex items-center justify-between" style={{ background: 'var(--md-sys-color-surface-container-high)' }}>
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider"
                      style={tr.medal !== 'none' ? { background: 'var(--md-sys-color-tertiary-container)', color: 'var(--md-sys-color-on-tertiary-container)' } : { background: 'var(--md-sys-color-surface-container-highest)', color: 'var(--md-sys-color-on-surface-variant)' }}>
